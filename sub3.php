...
<head>
<script defer>
    sub4 = localStorage.getItem('scores2');
    if (!sub4) {
...
<body>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", () => {
            ...
            document.querySelector("#tampil-nomor p:nth-of-type(2)").innerHTML = 1; // Replace 3 with your desired number
            ...
            function updateTampilanJumlah() {
                document.getElementById("jumlah-benar").innerHTML = jBenar3;
                document.getElementById("jumlah-salah").innerHTML = jSalah3;
            }

            // Fungsi untuk menyimpan data ke localStorage
            // Fungsi untuk menyimpan data ke localStorage
            function simpanKeLocalStorage() {
                localStorage.setItem('jBenar3', jBenar3);
                localStorage.setItem('jSalah3', jSalah3);
                localStorage.setItem('scores3',  JSON.stringify(scores3));
            }

            // Fungsi untuk mengambil data dari localStorage
            function ambilDariLocalStorage() {
                const jBenarLocal = localStorage.getItem('jBenar3');
                const jSalahLocal = localStorage.getItem('jSalah3');
                const scores3Local = localStorage.getItem('scores3');

                // Periksa apakah data tersedia di localStorage
                if (jBenarLocal !== null && jSalahLocal !== null && scores3Local !== null && scores3Local.trim() !== "") {
                        jBenar3 = parseInt(jBenarLocal);
                        jSalah3 = parseInt(jSalahLocal);
                        scores3 = JSON.parse(scores3Local); // Parse JSON string to array
  ...
  //              timer.innerHTML = (`0${waktu / 60}`).slice(-2) + ":00";
            ...
            function decrement() {
                ...
                if (count < 0) {
                    document.location.href = 'sub4.php'
                } else {
            ...
            document.querySelectorAll("#tombol-nomor div").forEach((tn) => {
                tn.addEventListener("click", async () => {
                    ...
                    const angkaArray = [0, 5, 4, 6, 4, 6, 5, 8, 4, 9, 5, 4, 6, 2, 0, 4, 5, 9, 6, 6, 7, 9, 5];
        ...
        localStorage.jBenar3 = 0;
       localStorage.jSalah3 = 0;
       localStorage.scores3 =[];
    </script>
