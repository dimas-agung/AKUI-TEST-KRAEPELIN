...
<head>
<script defer>
    sub4 = localStorage.getItem('scores3');
    if (!sub4) {
    ...
    <script>
        document.addEventListener("DOMContentLoaded", () => {
            ...
            function updateTampilanJumlah() {
                document.getElementById("jumlah-benar").innerHTML = jBenar4;
                document.getElementById("jumlah-salah").innerHTML = jSalah4;
            }

            // Fungsi untuk menyimpan data ke localStorage
            function simpanKeLocalStorage() {
                localStorage.setItem('jBenar4', jBenar4);
                localStorage.setItem('jSalah4', jSalah4);
                localStorage.setItem('scores4',  JSON.stringify(scores4));
            }

            // Fungsi untuk mengambil data dari localStorage
            function ambilDariLocalStorage() {
                const jBenarLocal = localStorage.getItem('jBenar4');
                const jSalahLocal = localStorage.getItem('jSalah4');
                const scores4Local = localStorage.getItem('scores4');

                // Periksa apakah data tersedia di localStorage
                if (jBenarLocal !== null && jSalahLocal !== null && scores4Local !== null && scores4Local.trim() !== "") {
                        jBenar4 = parseInt(jBenarLocal);
                        jSalah4 = parseInt(jSalahLocal);
                        scores4 = JSON.parse(scores4Local); // Parse JSON string to array
     ...
     //           timer.innerHTML = (`0${waktu / 60}`).slice(-2) + ":00";
            ...
            function decrement() {
                ...
                if (count < 0) {
                    document.location.href = 'sub5.php'
                } else {
            ...
            document.querySelectorAll("#tombol-nomor div").forEach((tn) => {
                tn.addEventListener("click", async () => {
                    ...
                    const angkaArray = [8, 5, 1, 4, 2, 2, 8, 3, 3, 5, 6, 4, 0, 8, 7, 9, 5, 9, 5, 5, 1, 6, 5];
        ...
        localStorage.jBenar4 = 0;
       localStorage.jSalah4 = 0;
       localStorage.scores4 =[];
    </script>
</body>
