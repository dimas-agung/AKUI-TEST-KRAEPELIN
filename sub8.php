...
<head>
<script defer>
    sub4 = localStorage.getItem('scores7');
    if (!sub4) {
...
<body>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", () => {
            ...
            document.querySelector("#tampil-nomor p:nth-of-type(5)").innerHTML = 1; // Replace 9 with your desired number
            let jBenar8 = 0;
            let jSalah8 = 0;

            // Fungsi untuk update tampilan jumlah benar dan salah
            function updateTampilanJumlah() {
                document.getElementById("jumlah-benar").innerHTML = jBenar8;
                document.getElementById("jumlah-salah").innerHTML = jSalah8;
            }

            // Fungsi untuk menyimpan data ke localStorage
            function simpanKeLocalStorage() {
                localStorage.setItem('jBenar8', jBenar8);
                localStorage.setItem('jSalah8', jSalah8);
                localStorage.setItem('scores8',  JSON.stringify(scores8));
            }

            // Fungsi untuk mengambil data dari localStorage
            function ambilDariLocalStorage() {
                const jBenarLocal = localStorage.getItem('jBenar8');
                const jSalahLocal = localStorage.getItem('jSalah8');
                const scores8Local = localStorage.getItem('scores8');

                // Periksa apakah data tersedia di localStorage
                if (jBenarLocal !== null && jSalahLocal !== null && scores8Local !== null && scores8Local.trim() !== "") {
                        jBenar8 = parseInt(jBenarLocal);
                        jSalah8 = parseInt(jSalahLocal);
                        scores8 = JSON.parse(scores8Local); // Parse JSON string to array
            ...
            function decrement() {
                ...
                if (count < 0) {
                    document.location.href = 'sub9.php'
                } else {
            ...
            document.querySelectorAll("#tombol-nomor div").forEach((tn) => {
                tn.addEventListener("click", async () => {
                    ...
                    const angkaArray = [9, 5, 5, 5, 7, 5, 3, 5, 7, 9, 8, 9, 7, 5, 4, 5, 8, 6, 4, 6, 5, 0, 4];
